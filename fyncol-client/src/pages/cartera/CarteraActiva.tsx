  ...
  return (
    <div className="relative flex h-[calc(100vh-64px)] w-full bg-[#0B0B12] overflow-hidden">
      ...
      <div className={`absolute md:relative inset-0 md:inset-auto w-full md:w-[420px] lg:w-[480px] h-full flex flex-col bg-[#05050A] border-r border-white/10 shrink-0 shadow-2xl transition-transform duration-300 ease-in-out z-[100] md:z-10 ${mobileView === 'MAP' ? '-translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto pt-3 pb-24 md:pb-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-track]:bg-transparent">
          {isLoading ? (
            <div className="p-10 flex flex-col items-center justify-center gap-3 text-slate-400"><FiLoader className="animate-spin text-blue-500" size={24} /><span className="text-sm">Sincronizando ruta...</span></div>
          ) : filteredData.length === 0 ? (
            <div className="p-10 text-center text-slate-500 text-sm">No hay clientes que coincidan con los filtros.</div>
            ...
            filteredData.map((client: any, index: number) => {
              ...
              return (
                <div 
                  ...
                  className={`flex flex-col p-4 mb-3 mx-4 rounded-2xl border backdrop-blur-md shadow-lg transition-all cursor-pointer group ${cuotaActiva?.status === 'RENEGOTIATED' ? 'bg-orange-500/5 border-orange-500/20 hover:bg-orange-500/10' : 'bg-[#0B0B12]/80 border-white/10 hover:border-white/20 hover:bg-[#12121A]'}`}
                >
                  <div className="flex w-full items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="font-semibold text-sm text-white truncate pr-2">{client.name}</h3>
                        {hasPaidToday || prestamoTerminado ? (
                           <FiCheckCircle className="text-emerald-500 shrink-0 mt-0.5 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" size={16} title="Pagado hoy / Terminado" />
                        ) : cuotaActiva?.status === 'OVERDUE' ? (
                           <FiAlertTriangle className="text-red-500 shrink-0 mt-0.5 drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]" size={16} title="En Mora" />
                        ) : cuotaActiva?.status === 'RENEGOTIATED' ? (
                           <FiClock className="text-orange-400 shrink-0 mt-0.5 drop-shadow-[0_0_8px_rgba(249,115,22,0.5)]" size={16} title="Mora Renegociada" />
                        ) : (
                           <div className="w-4 h-4 rounded-full border-2 border-slate-600 shrink-0 mt-0.5"></div>
                        )}
                      </div>
                      
                      <div className="text-xs text-slate-400 flex items-center gap-1 truncate"><FiMapPin className="shrink-0" size={10}/> {client.address}</div>
                    </div>
                  </div>

                  {/* VISUALIZADOR DE CUOTAS CIRCULARES ESTILO TARJETA PREMIUM */}
                  <div className="ml-8 mt-3 mb-1 p-2.5 bg-black/30 rounded-xl border border-white/5 flex flex-wrap gap-2 items-center">
                    {loan.installmentDetails?.map((inst: any) => {
                       let dotClass = "bg-white/10 border border-white/20"; 
                       if (inst.status === 'PAID') dotClass = "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]";
                       else if (inst.status === 'PARTIAL') dotClass = "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]";
                       else if (inst.status === 'OVERDUE') dotClass = "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]";
                       else if (inst.status === 'RENEGOTIATED') dotClass = "bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]";

                       return (
                          <div 
                             key={inst.id}
                             title={`Cuota ${inst.installmentNumber} - ${traducirEstado(inst.status)}`}
                             className={`w-2.5 h-2.5 rounded-full ${dotClass} transition-all`}
                          />
                       );
                    })}
                  </div>
                  ...
                  <div className="ml-8 mt-1">
                    {cuotaActiva && !prestamoTerminado && (
                      <div className={`mt-2 rounded-xl p-3 border shadow-inner ${cuotaActiva.status === 'RENEGOTIATED' ? 'bg-orange-500/10 border-orange-500/20' : 'bg-black/20 border-white/5'}`}>
                        <div className="flex justify-between items-center mb-2">
                           <span className={`text-sm font-bold ${cuotaActiva.status === 'RENEGOTIATED' ? 'text-orange-400' : 'text-white'}`}>${faltanteActual.toLocaleString('es-CO')}</span>
                           <span className="text-[9px] font-bold px-2 py-1 rounded-md text-blue-400 bg-blue-500/10 uppercase tracking-widest border border-blue-500/20">Cuota {cuotaActiva.installmentNumber}</span>
                        </div>
                        ...
                        {cuotaActiva.actionDescription && (
                          <div className="mb-2 text-[10px] font-medium text-slate-400 bg-black/40 p-1.5 rounded-lg flex items-center gap-1.5 italic border border-white/5">
                            <FiInfo className="shrink-0 text-blue-400" size={12} />
                            <span className="truncate">{cuotaActiva.actionDescription}</span>
                          </div>
                        )}
                            ...
                            <div className="flex gap-1.5 mt-3">
                              {cuotaActiva.status !== 'RENEGOTIATED' && (
                                <button 
                                  ...
                                  className="flex-[2] bg-blue-600 hover:bg-blue-500 py-2 rounded-lg text-white text-xs font-bold flex justify-center items-center transition-all shadow-lg shadow-blue-900/20"
                                >
                                  PAGAR
                                </button>
                              )}
                              ...
                              {cuotaActiva.status !== 'RENEGOTIATED' && (
                                <button 
                                  ...
                                  className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 py-2 rounded-lg text-xs font-bold flex justify-center items-center transition-all"
                                >
                                  MORA
                                </button>
                              )}
