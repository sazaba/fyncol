...
export default function NuevoCredito() {
  ...
  // --- LÓGICA DE DÍA DE INICIO DE COBRO ---
  useEffect(() => {
    let daysToAdd = rutaAsignada?.firstPaymentDelay !== undefined ? Number(rutaAsignada.firstPaymentDelay) : 1; 
  ...
  }, [formData.periodicity, rutaAsignada?.country, rutaAsignada?.firstPaymentDelay]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (maxInstallmentsLimit > 0 && Number(formData.installments) > maxInstallmentsLimit) {
        throw new Error(`El número de cuotas (${formData.installments}) supera el límite permitido para esta ruta (${maxInstallmentsLimit}).`);
      }
      ...
      openAlert({
        ...
        onConfirm: () => {
          ...
          setFormData({ 
            ...
            amount: '', installments: '', interestRate: '', 
            periodicity: 'DIARIO', firstPaymentDate: getRouteDateStr(rutaAsignada?.country, rutaAsignada?.firstPaymentDelay ?? 1)
          });
  ...
  const maxInstallmentsLimit = Number(rutaAsignada?.maxInstallments) || 0;
  const isOverInstallmentsLimit = maxInstallmentsLimit > 0 && Number(formData.installments) > maxInstallmentsLimit;
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0B0B12]/80 backdrop-blur-sm border border-white/5 rounded-3xl p-6 flex flex-col h-full shadow-xl">
          ...
          <div className="space-y-5 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="flex items-center justify-between text-[10px] font-semibold text-slate-400 mb-1.5 uppercase tracking-widest">
                  <span>Cuotas</span>
                  {maxInstallmentsLimit > 0 && (
                    <span className="text-emerald-500/80">Máx: {maxInstallmentsLimit}</span>
                  )}
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
                      <FiLayers size={14} className={isOverInstallmentsLimit ? 'text-red-400' : ''} />
                  </div>
                  <input 
                    required 
                    type="number" 
                    name="installments" 
                    max={maxInstallmentsLimit > 0 ? maxInstallmentsLimit : undefined}
                    value={formData.installments} 
                    onChange={handleChange} 
                    disabled={!rutaAsignada} 
                    className={`w-full bg-[#05050A]/50 border rounded-2xl pl-9 pr-4 py-3.5 text-sm text-white placeholder-slate-600 focus:outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-inner ${
                        isOverInstallmentsLimit 
                ...
                {isOverInstallmentsLimit && (
                  <p className="text-[10px] text-red-400 mt-2 flex items-center gap-1.5 font-medium">
                    <FiAlertTriangle size={12} /> Excede el máximo permitido por la ruta.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  {/* === SE MODIFICA EL INPUT PARA BLOQUEARLO === */}
                  <input 
                    required 
                    type="date"
                    ...
                    disabled={true} 
                    className="w-full bg-[#05050A]/50 border border-white/10 rounded-2xl pl-9 pr-2 py-3.5 text-xs text-slate-400 focus:outline-none transition-all cursor-not-allowed opacity-60 shadow-inner [&::-webkit-calendar-picker-indicator]:opacity-50 [&::-webkit-calendar-picker-indicator]:cursor-not-allowed [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 bg-gradient-to-br from-blue-900/10 to-blue-600/5 border border-blue-500/20 rounded-3xl p-6 flex flex-col gap-5 shadow-[inset_0_0_20px_rgba(37,99,235,0.05)] relative">
              ...
              <button 
                ...
                disabled={creditMetrics.schedule.length === 0 || isOverInstallmentsLimit}
          ...
          <div className="pt-6 mt-6 border-t border-white/5">
            <button 
              type="submit" 
              disabled={isSubmitting || !rutaAsignada || isOverInstallmentsLimit} 
