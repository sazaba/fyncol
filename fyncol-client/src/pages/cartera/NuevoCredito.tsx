...
import { 
  FiMapPin, FiUploadCloud, FiSave, FiUser, FiDollarSign, 
  FiMap, FiBriefcase, FiAlertTriangle, FiX, FiLoader, FiCalendar, FiList, FiCreditCard, FiPhone, FiChevronDown, 
  FiSearch, FiLayers // <-- Añadido FiLayers para el icono del tope de cuotas
} from 'react-icons/fi';
...
export default function NuevoCredito() {
  ...
  useEffect(() => {
    ...
    // Solo aplicamos el salto a 15 o 30 días si el delay inicial es mayor a 0 
    // y no es un crédito de cobro inmediato.
    if (formData.periodicity === 'QUINCENAL') daysToAdd = 15;
    if (formData.periodicity === 'MENSUAL') daysToAdd = 30;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Bloqueo antes de enviar si las cuotas superan el máximo permitido.
  ...
  // --- LÓGICA DE UI PARA TOPES DE RUTA ---
  const maxLoanLimit = Number(rutaAsignada?.maxLoanPerClient) || 0;
  const isOverLimit = maxLoanLimit > 0 && Number(formData.amount) > maxLoanLimit;
          ...
          <div className="space-y-5 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {/* --- INDICADOR VISUAL DE BLOQUEO DE CUOTAS --- */}
                {isOverInstallmentsLimit && (
